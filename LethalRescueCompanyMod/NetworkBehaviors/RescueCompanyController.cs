...
public class RescueCompanyController : NetworkBehaviour
{
    ...
    internal ManualLogSource log =
        Logger.CreateLogSource("LethalRescueCompanyPlugin.NetworkBehaviors.RescueCompanyController");
    ...
    public void ServerEventHandler(Event eventName)
    {
        ...
        switch (eventName.command)
        {
            ...
            case CommandContract.Command.SpawnSpider:
                ...
                {
                    log.LogWarning(
                        "ServerEventHandler ignoring spawn spider because localplayercontroller is not host");
                }
    ...
    public void ServerSpawnAsset(string identifier, Vector3 position)
    {
        ...
        var go = Instantiate(AssetManager.GetAssetByKey(identifier), position, Quaternion.identity);
        if (IsServer)
        {
    ...
    public void SpawnViaComponent(string identifier, PlayerControllerB p)
    {
        ...
        {
            case "spider":
                var spiderSpawnBehavior = p.GetComponent<SpiderSpawnBehavior>();
    ...
    public void ReplacementBody(int playerId, Vector3 position)
    {
        ...
        var gameObject = Instantiate(AssetManager.GetAssetByKey("CubePrefab"), position, Quaternion.identity);
