...
{
    internal static ManualLogSource log = Logger.CreateLogSource("LethalRescueCompanyPlugin.Patches.RevivableTrait");
    ...
    private readonly bool isDebug = Settings.IsDebug;
    ...
    public void revivePlayer(bool removeBody)
    {
        ...
        if (Settings.DebugAddRevive && !isUsed && !isRespawning)
            StartCoroutine(WaitFiveSecondsAndRevive(playerControllerB,
                playerControllerB.playersManager.outsideShipSpawnPosition.position, false));
        ...
        if (grabbableObject.isInShipRoom && playerControllerB.playersManager != null)
        {
            if (!grabbableObject.isHeld)
                if (!isRespawning && !isUsed)
                {
                    ...
                    StartCoroutine(WaitFiveSecondsAndRevive(playerControllerB,
                        playerControllerB.playersManager.playerSpawnPositions[0].position, removeBody));
                }
        }
        else
        {
            log.LogDebug(
                $"revive conditions unmet: {grabbableObject.isInShipRoom}, {playerControllerB.playersManager} ");
        }
    }
    ...
    public void playerIsDeadInShipAndRevivable(DeadBodyInfo deadBodyInfo, StartOfRound playersManager)
    {
        try
        {
            ...
            if (!deadBodyInfo.grabBodyObject.isHeld) // might be 6
            {
                if (!isRespawning)
                    if (Settings.IsDebug)
                        log.LogInfo("trait found, reviving coroutine");
